...
angular.module("calculator").directive "calculator", ->
    ...
    init = ($api, notifications, $scope, $state, $stateParams, $timeout, configCalculator, billingCycleDiscount, components) ->
        ...
        initOrderComponents = (components, config)->
            ...
            angular.forEach components, (component, componentId) ->
                ...
                if angular.isArray(configData[id])
                    defaultOrder[category][name] = configData[id][0]
                else
                    # числовое значение в опции компонента
                    defaultOrder[category][name] = configData[id]
    ...
    restrict: "AE"
    template: require './calculator.jade'
    scope:
        currency: "@"
        groups: "@"
        currencyId: "@"
